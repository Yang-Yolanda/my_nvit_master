...
class pytorch_pruning(object):
    ...
    def compute_latency(self, emb, qk_head, qk, v, mlp):
        # 🔥 快速修复
        if not isinstance(self.latency_look_up_table, dict) or len(self.latency_look_up_table) == 0:
            return 0.0
        
        point = [emb, qk_head, qk, v, mlp]
        
        # 尝试查找
        key = "_".join(map(str, point))  # 或者用其他格式
        return self.latency_look_up_table.get(key, 0.0)
    ...
    def compute_saliency(self):
        ...
        if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
            ...
            # 🔥 修改为动态计算大小
            num_layers = len([l for l in range(len(self.pruning_gates)) if l % 4 == 0])
            model_dim_size = 36 + num_layers  # attention (36) + MLP (num_layers)
            model_dim = np.zeros((1, model_dim_size))
            ...
            if hasattr(self, 'model'):
                if hasattr(self.model, 'backbone'):
                    if hasattr(self.model.backbone, 'embed_dim'):
                        emb = self.model.backbone.embed_dim
                    elif hasattr(self.model.backbone, 'patch_embed'):
                        emb = self.model.backbone.patch_embed.proj.out_channels
            
            # 如果还是 None，从第一个可剪枝层推断
            if emb is None:
                for layer, if_prune in enumerate(self.prune_layers):
                    if if_prune and len(self.parameters[layer].shape) >= 2:
                        emb = self.parameters[layer].shape[1]
                        break
            
            # 最后的默认值
            if emb is None:
                emb = 768
                print(f"⚠️ Could not determine embedding dimension, using default: {emb}")
            else:
                print(f"✅ Detected embedding dimension: {emb}")
            ...
            if self.train_writer is not None:
                self.train_writer.add_scalar('dimension/EMB', emb, self.pruning_iterations_done)
            ...
            for layer, if_prune in enumerate(self.prune_layers):
                ...
                if layer%4 == 1:
                    ...
                    mlp = np.nonzero(self.pruning_gates[layer])[0].size
                    idx = layer // 4 + 36
                    if idx < model_dim.shape[1]:
                        model_dim[0, idx] = mlp
                    else:
                        print(f"⚠️ MLP index {idx} out of bounds (max: {model_dim.shape[1]-1}), skipping layer {layer}")
                    
                # 🔥 只在有效范围内计算 latency
                    if idx < model_dim.shape[1] and self.latency_regularization > 0:
    ...
    def do_step(self, loss=None, optimizer=None, neurons_left=0, training_acc=0.0):
        ...
        if self.maximum_pruning_iterations <= self.pruning_iterations_done:
            ...
            self.is_finished = True
            return -1
        if not hasattr(self, 'res_pruning'):
            self.res_pruning = 1  # 初始化为 1，允许剪枝

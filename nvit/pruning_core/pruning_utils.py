...
# pruning_core/pruning_utils.py

import json
import torch.nn as nn

def initialize_pruning_engine(named_modules, pruning_config, exp_name, 
                               prune_by="predefined", tensorboard=True,
                               latency_regularization=0.0, latency_target=0.0,
                               latency_look_up_table=''):
    """
    初始化剪枝引擎
    """
    from .pruning_engine_general import pytorch_pruning
    
    # 加载配置文件作为 pruning_settings
    with open(pruning_config, 'r') as f:
        pruning_settings = json.load(f)
    
    # ============================================
    # 🔧 构建 for_pruning_parameters 列表
    # 格式可能是 [(name, module), ...] 或 [module, ...]
    # ============================================
    for_pruning_parameters = []
    
    for name, module in named_modules:
        if getattr(module, 'is_prunable', False):
            # 尝试格式 1: (name, module) 元组
            for_pruning_parameters.append((name, module))
            print(f"📌 Registered: {name}")
    
    print(f"📊 Total pruning parameters: {len(for_pruning_parameters)}")
    
    if len(for_pruning_parameters) == 0:
        print("⚠️ 警告：没有找到可剪枝的层！")
    
    # 创建剪枝引擎
    pruning_engine = pytorch_pruning(
        for_pruning_parameters=for_pruning_parameters,
        pruning_settings=pruning_settings,
        log_folder=exp_name,
        latency_regularization=latency_regularization,
        latency_target=latency_target,
        latency_look_up_table=latency_look_up_table
    )

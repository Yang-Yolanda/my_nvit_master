...
def get_smpl_adjacency_matrix(directed=True, add_self_loops=True, mode='outward'):
    """
    Generates the Adjacency Matrix for the SMPL skeleton.
    
    Args:
        directed (bool): Deprecated, kept for backward compat. If True -> 'outward'.
        add_self_loops (bool): If True, adds diagonal I to A.
        mode (str): 'outward' (Parent->Child), 'inward' (Child->Parent), 'undirected'.
                    Overrides 'directed' if explicitly provided? 
                    Logic: if directed is False, mode is forced to 'undirected'.
    ...
    # Resolve mode based on legacy flag
    if not directed:
        resolved_mode = 'undirected'
    else:
        resolved_mode = mode
    ...
    for child, parent in SMPL_PARENTS.items():
        ...
        if resolved_mode == 'outward':
            # Edge: Parent -> Child
            adj[p_idx, c_idx] = 1.0
        elif resolved_mode == 'inward':
            # Edge: Child -> Parent
            adj[c_idx, p_idx] = 1.0
        else: # undirected
            adj[p_idx, c_idx] = 1.0

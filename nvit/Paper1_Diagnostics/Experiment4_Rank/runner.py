...
def run_experiment_4(args):
    """
    Experiment 4: Attention Entropy & Gaze (Globality vs locality)
    """
    model_name = args.model
    output_root = "results"
    
    logger.info(f"Running Experiment 4 (Rank) for model: {model_name}")
    ...
    # 4. Filter groups for Experiment 4 (Usually just Control is enough for diagnostic curves)
    ...
    logger.info(f"Experiment 4 Complete. Results in {lab.output_dir}/results.csv")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Paper 1 Experiment 4: Effective Rank Analysis')

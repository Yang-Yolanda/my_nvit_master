...
def load_target_model(model_name, ckpt_path=None):
    ...
    if '4d-humans' in model_name.lower() or 'hmr2' in model_name.lower():
        ...
        camerahmr_root = '/home/yangz/NViT-master/external_models/CameraHMR'
        ...
        # Setup HSMR path
        hsmr_root = '/home/yangz/NViT-master/external_models/HSMR'
        ...
        phmr_root = '/home/yangz/NViT-master/external_models/PromptHMR'
...
def run_experiment_1(args):
    """
    Experiment 1: Attention Entropy & Gaze (Globality vs locality)
    Target: Control Group Only (Baseline Analysis)
    """
    model_name = args.model
    # Output to Experiment1_Entropy/results/ModelName
    script_dir = Path(__file__).resolve().parent
    output_root = script_dir / "results"
    
    logger.info(f"Running Experiment 1 (Entropy) for model: {model_name}")
    ...
    # --- Experiment 3 Logic: Ablation Groups ---
    logger.info("--- Starting Experiment 3: Masking & Intervention Analysis ---")
    
    # Do not clear groups, allow the 6 T2 groups to run
    ...
    logger.info(f"Experiment 1 Complete. Results in {lab.output_dir}/results.csv")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Paper 1 Experiment 1: Entropy & Gaze')
    parser.add_argument('--model', type=str, default='4D-Humans', help='Model to diagnose')
    parser.add_argument('--ckpt', type=str, default=None, help='Path to checkpoint')
    # Default to 50 batches for analysis (enough for convergence)

...
def run_experiment_5(args):
    """
    Experiment 5: Attention Entropy & Gaze (Globality vs locality)
    """
    model_name = args.model
    output_root = "results"
    
    logger.info(f"Running Experiment 5 (Hybrid Sim) for model: {model_name}")
    ...
    # 4. Filter groups for Experiment 5 (Only Hybrid-Sim and Control)
    lab.groups = {
        'Control': lab.groups['Control'],
        'Hybrid-Sim': lab.groups['Hybrid-Sim']
    }
    
    # 5. Run Experiment 5
    lab.run_experiment(data_loader, evaluator, dataset_cfg, num_batches=args.num_batches)
    
    logger.info(f"Experiment 5 Complete. Results in {lab.output_dir}/results.csv")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Paper 1 Experiment 5: Hybrid Simulation')
